...
module.exports = async function swapTokens(ctx, fromToken, toToken, amount, recipient) {
  try {
    ...
    const amountIn = bestPath.amountIn.toString(); // Use decimal values as strings
    const minAmountOut = bestPath.amountOut.toString(); 
    const deadline = Math.floor(Date.now() / 1000) + 60 * 10; // Unix timestamp
    ...
    const tx = await contract.swapExactInput(
      ...
      poolLengths,
      fees,
      swapData // Ensure correct format
    ).send({
      feeLimit: 100_000_000,
      shouldPollResponse: true
    });
